...
export async function update(req:Request, res:Response) {
    ...
    if(!(email && username)) return res.status(400).json({ message: 'Geef alle gegevens mee.' });

    if (!isEmail(email)) return res.status(400).json({ message: 'Geef een geldig-email adres mee.' });
    if (username.length < 2 || username.length > 30) return res.status(400).json({ message: 'Geef een geldige gebruikersnaam mee.' });
...
export async function updatePassword(req:Request, res:Response) {
    ...
    if(!(oldPassword && newPassword && confirmPassword)) return res.status(400).json({ message:'Geef alle gegevens mee.' });
